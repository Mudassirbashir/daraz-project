...
import { X, PackageCheck, CheckCircle2, Barcode, AlertCircle, RefreshCw, Printer, ArrowRight, Check, Info } from "lucide-react";
...
export function PackingModal({
  ...
  const processPackedMatch = (match: ScanMatchItem | any) => {
    setErrorMessage("");
    if (packingState === "error") setPackingState("idle");
    ...
    const matchedIdx = items.findIndex((it: any) => {
      const itOrdItemId = (it.order_item_id || it.id || "").toLowerCase();
      const itSku = (it.seller_sku || it.sku || "").toLowerCase();
      return (
    ...
    if (matchedIdx !== -1) {
      setCheckedItems((prev) => ({ ...prev, [matchedIdx]: true }));
      setScanInfoMessage(
        `✓ Package Verified [${matchType.toUpperCase()}]: ${prodName} (${items[matchedIdx].seller_sku || items[matchedIdx].sku}) checked.`
      );
      setBarcodeInput("");
    } else {
      ...
      setScanInfoMessage(
        `ℹ Scanned Product Found [${matchType.toUpperCase()}]: "${prodName}" associated with Order #${ordId} (${storeName}). Not part of this package.`
      );
      setBarcodeInput("");
    }
  };
  ...
  const handleScanBarcode = async (e: React.FormEvent) => {
    ...
    const term = normalizeScanValue(barcodeInput);
    if (!term) return;
    ...
    try {
      ...
      processPackedMatch(matchItem);
    } catch (err: any) {
      setErrorMessage(err.message || "An error occurred during barcode scanning.");
    } finally {
      setIsScanning(false);
    }
  };
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
        <div className="relative w-full max-w-2xl rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-2xl space-y-6 max-h-[92vh] overflow-y-auto text-xs">
          ...
          {packingState !== "packed" && !order.is_packed && (
            <form onSubmit={handleScanBarcode} className="flex items-center space-x-2">
              ...
              <button
                type="submit"
                disabled={isScanning}
                className="rounded-xl bg-slate-950 dark:bg-slate-800 px-4 py-2 font-bold text-white hover:bg-slate-800 apple-press disabled:opacity-50 inline-flex items-center space-x-1"
              >
                {isScanning ? (
                  <>
          ...
          {/* Error message */}
          {(errorMessage || packingState === "error") && (
            <div className="p-3 rounded-xl bg-red-50 text-red-800 dark:bg-red-900/30 dark:text-red-300 border border-red-200 flex items-center space-x-2">
              ...
              <span>{errorMessage || "Packing failed."}</span>
            </div>
          )}
