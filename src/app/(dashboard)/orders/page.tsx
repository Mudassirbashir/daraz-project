...
export default function OrdersPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s === "shipped") {
      return (
        <span className="inline-flex items-center space-x-1 rounded-md bg-indigo-50 px-2 py-0.5 text-[11px] font-bold text-indigo-700 border border-indigo-200">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Daraz Seller Center — Orders ERP</h1>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
        <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm">
          ...
          <p className="mt-1 text-xl font-bold text-slate-900">{metrics.totalOrders}</p>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm">
          <span className="text-[10px] font-bold uppercase text-slate-500">Pending</span>
          <p className="mt-1 text-xl font-bold text-amber-600">{metrics.pending}</p>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm">
          <span className="text-[10px] font-bold uppercase text-slate-500">Ready to Ship</span>
          <p className="mt-1 text-xl font-bold text-blue-600">{metrics.readyToShip}</p>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm">
          <span className="text-[10px] font-bold uppercase text-slate-500">Delivered</span>
          <p className="mt-1 text-xl font-bold text-emerald-600">{metrics.delivered}</p>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm">
          <span className="text-[10px] font-bold uppercase text-slate-500">Cancelled / Failed</span>
          <p className="mt-1 text-xl font-bold text-red-600">{metrics.canceled + metrics.failed}</p>
        </div>

        <div className="rounded-xl border border-orange-200 bg-orange-50/50 p-3.5 shadow-sm">
          <span className="text-[10px] font-bold uppercase text-orange-700">Today's Revenue</span>
          <p className="mt-1 text-xl font-bold text-orange-900">{todaysRevenueFormatted}</p>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 pb-2 overflow-x-auto text-xs">
        ...
        <button
          ...
          className={`px-4 py-2 font-bold rounded-lg transition-all ${
            statusFilter === "pending" ? "bg-amber-500 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 font-bold rounded-lg transition-all ${
            statusFilter === "ready_to_ship" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 font-bold rounded-lg transition-all ${
            statusFilter === "delivered" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 font-bold rounded-lg transition-all ${
            statusFilter === "canceled" ? "bg-red-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
          }`}
        >
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {loading ? (
                  ...
                  return (
                    <tr key={ord.id} className="hover:bg-slate-50/50 transition-colors">
                      {columnVisibility.orderId && (
                        <td className="px-4 py-3 font-mono">
                          <div className="flex items-center space-x-1">
                            <span className="font-bold text-slate-900">#{ord.daraz_order_id}</span>
                            <button
                      ...
                      {columnVisibility.phone && (
                        <td className="px-4 py-3 font-mono text-slate-600">{ord.customer_phone || "N/A"}</td>
                      )}
                      ...
                      {columnVisibility.shipping && (
                        <td className="px-4 py-3 text-slate-700 font-medium">
                          {ord.shipping_provider || "Daraz Express"}
                        </td>
                      )}
          ...
          <div className="p-12 text-center text-xs text-slate-500 space-y-2">
            ...
            <p className="font-medium text-slate-700">No orders found matching your current filter.</p>
